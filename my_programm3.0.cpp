...
int key(vector<string> c, string n) {//num of elem
...
bool keey(vector<string> l, string k) {//elem
...
bool kei(vector<int>v, int i) {//same bu to int
...
int keei(vector<int>v, int i) { //same b to int
    for (int ij = 0; ij < v.size(); ij++) {
        if (v[ij] == i) {
            return ij;
        }
...
int main()
{
    ...
    vector<pair<int, int>> oplatea;//oplata day sum 
    ...
    while (command != "stop") {
        ...
        if (command == "show_kruz") {
            if (childrens.empty()) {
                cout << "\nnnnno detei neet tut!\n";
                break;
            }
            else {
                // if (childrens.size() > 1) {
                //     cout << "\nenter name of child\n";
                //     cin >> command;
                if (keey(names, command) == f) {
                    cout << "\nno kido called like this. try again. \n";
                    break;

                }
                else {
                    cout << "\n\t\t^KRUZKI^\n";
                    for (int yup = 0; yup < namesk.size(); yup++) {
                        cout << yup + 1 << " " << namesk[yup] << endl;
                    }
                }
                //}
            }
